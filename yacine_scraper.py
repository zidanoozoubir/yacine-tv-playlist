...
# 4. دالة الفرز والتصنيف الدقيق المحدثة بالكامل
...
# 5. جلب وتنقية القنوات مع المعالجات الخاصة لمنع أخطاء 403 و 200
def fetch_and_process_app2(session):
    ...
    print("🚀 جاري جلب قنوات التطبيق الثاني وتصنيفها لصفحة s1.m3u...")
    try:
                ...
                if line_str.startswith("#EXTINF:"):
                    ...
                    if current_extinf:
                        ...
                        if group_title:
                            ...
                            # 🛠️ معالجة حاسمة: تحويل امتداد الروابط تلقائياً إلى .ts بدلاً من .m3u8 لمنع التقطيع
                            final_url = line_str.replace(".m3u8", ".ts")
                            
                            # توحيد المسار والسيرفر المستقر
                            final_url = final_url.replace("217.60.15.177:8080", "185.191.126.127:8080")

                            if final_url in seen_urls:
                                continue

                            # ترويسة تشغيل متوافقة لمنع الحظر
...
# 6. التنفيذ المباشر وحفظ البيانات على Gist
def main():
    ...
    # 🛡️ درع الحماية: إن كان عدد القنوات المجلوبة صفر بسبب الصيانة، نلغي التحديث للحفاظ على القنوات القديمة
    if total_count == 0:
        print("\n🛡️ [درع الحماية]: تم رصد توقف/صيانة في سيرفر المصدر!")
        print("🛡️ إلغاء العملية للحفاظ على القنوات القديمة شغالّة بدون مسح.")
        return

    # ترتيب المجموعات كاملاً المحدث بجميع الإضافات الجديدة
    preferred_order = [
        "BEIN SPORT AR",
        "TOD CHANNEL",
        "ALWAN SPORT",
    ...
    try:
        ...
        if get_gist.status_code == 200:
            ...
            if patch_resp.status_code == 200:
                print(f"\n🎉 تم تحديث الـ Gist بنجاح لـ s1.m3u ({filename}) بإجمالي ({total_count}) قناة بدون تقطيع!")
            else:
