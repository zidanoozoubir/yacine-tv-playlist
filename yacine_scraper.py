...
# 2. تهيئة وتجهيز باقة قنوات LIVE المباشرة الجديدة بالترتيب الأول (بالأسماء الجديدة)
    ...
    try:
        ...
        if basha_response.status_code == 200:
            ...
            for channel in basha_channels:
                ...
                # تصفية قوية وجلب القنوات العربية والفرنسية فقط (استبعاد القنوات التركية، الألمانية وغيرها)
                is_arabic = any('\u0600' <= char <= '\u06FF' for char in channel_name)
                is_premium_ar = any(target in channel_name_lower for target in ["bein", "osn", "mbc", "ssc", "shahid", "art", "rotana", "al jazeera", "vip", "basha"])
                is_french = any(target in channel_name_lower for target in ["fr:", "fr ", "(fr)", "[fr]", "france", "canal", "rmc", "tf1", "m6", "ocs", "cine"])
                
                if is_arabic or is_premium_ar or is_french:
...
print(f"🎯 تم استخراج ({matched_count}) قناة عربية وفرنسية من الباشا تيفي بنجاح.")

# 4. جلب وتنسيق باقة قنوات ياسين تيفي (Yacine TV) - نفس كودك الأصلي تماماً
