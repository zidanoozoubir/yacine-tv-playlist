...
def extract_static_channels(m3u_content):
    ...
    # تم إضافة "majed-koora.live" و "LIVE" لمنع تكرار قنوات البث المباشر في القسم الثابت
    exclude_keywords = [
...
# 3. جلب وتصفية باقة قنوات الباشا تيفي (Al Basha TV) - جلب العربية والفرنسية فقط
    ...
    try:
        ...
        if basha_response.status_code == 200:
            ...
            for channel in basha_channels:
                ...
                # فحص وتصفية القنوات: جلب العربية والفرنسية فقط وتجاهل باقي اللغات
                is_arabic = any('\u0600' <= char <= '\u06FF' for char in channel_name)
                is_arabic_english = any(target in channel_name_lower for target in ["bein", "osn", "mbc", "ssc", "shahid", "art", "rotana", "al jazeera", "vip", "basha"])
                is_french = any(target in channel_name_lower for target in ["fr:", "fr ", "(fr)", "[fr]", "france", "canal", "rmc", "tf1", "m6", "ocs", "cine"])
                
                if is_arabic or is_arabic_english or is_french:
